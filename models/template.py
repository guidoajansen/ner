...
class Template(object):
    ...
    def add_summary(self):
        ...
        self.precision = tf.placeholder(tf.float32, shape=None, name='Precision')
        precision_sum = tf.summary.scalar('Precision', self.precision)

        self.recall = tf.placeholder(tf.float32, shape=None, name='Recall')
        recall_sum = tf.summary.scalar('Recall', self.recall)

        self.f1 = tf.placeholder(tf.float32, shape=None, name='F1')
        f1_sum = tf.summary.scalar('F1', self.f1)

        self.merged_performance = tf.summary.merge([accuracy_sum, precision_sum, recall_sum, f1_sum])
